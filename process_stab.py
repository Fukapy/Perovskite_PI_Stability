...
def onehot_vector_table(x_name, df):
    vector_table_data = pd.get_dummies(df[x_name], dtype=int).add_prefix(x_name+"_")
    # added "dtype=int"
...
def multihot_vector_table_1(x_name, df):
    ...
    for j in range(np.shape(single_split)[1]):
        stop = start + len(df)
        dummy = pd.get_dummies(single_split_reshape ,dtype=int).iloc[start:stop].reset_index(drop = True)
...
def multihot_vector_table_2(x_name,df):
    ...
    for j in range(np.shape(double_split)[1]):
        stop = start + len(df)
        dummy = pd.get_dummies(double_split_reshape,dtype=int).iloc[start:stop].reset_index(drop=True)
        small_box.append(dummy)
...
def multihot_vector_table_3(x_name,df):
    ...
    for j in range(np.shape(triple_split)[1]):
        stop = start + len(df)
        dummy = pd.get_dummies(triple_split_reshape,dtype=int).iloc[start:stop].reset_index(drop = True)
        small_box.append(dummy)
...
#     Save vectors with compressed data.
    vec2csr(vec = X,
            csr_file_name = f"data_stab/csr/{use_X}_sp{str(split_way)}_{per_elem_prop}_{fill_way}_csr.npz",
            columns_file_name = f"data_stab/csr/{use_X}_sp{str(split_way)}_{per_elem_prop}_{fill_way}_columns.npy")
