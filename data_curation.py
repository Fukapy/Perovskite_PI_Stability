...
==========================================================
Source: data/ZhangZ_PD.csv (The Perovskite Database)
Output: outputs/curated/Perovskite_5800data_addln.csv
        outputs/csr/all_sp1_oliynyk_zero_{csr,columns}.*
...
def curate_data():
    ...
    """
    # --- Load ---
    raw_df = pd.read_csv("data/ZhangZ_PD.csv")
    raw_df["Original_index"] = raw_df.index

    # Include both stability targets (TS80, TS80m) and PCE target
    target_cols = [raw_df["TS80"], raw_df["TS80m"]]
    if "JV_default_PCE" in raw_df.columns:
        target_cols.append(raw_df["JV_default_PCE"])

    df_1 = pd.concat(
        [raw_df["Original_index"], raw_df.iloc[:, 11:259]] + target_cols,
        axis=1,
    )
    ...
    # --- Save ---
    out_dir = "outputs/curated"
    os.makedirs(out_dir, exist_ok=True)
    df_5.to_csv(f"{out_dir}/Perovskite_5800data.csv", index=False)
    df_5["lnTS80m"] = np.log(df_5["TS80m"])
    # Ensure JV_default_PCE is numeric (non-numeric → NaN, kept as-is)
    if "JV_default_PCE" in df_5.columns:
        df_5["JV_default_PCE"] = pd.to_numeric(df_5["JV_default_PCE"], errors="coerce")
    df_5.to_csv(f"{out_dir}/Perovskite_5800data_addln.csv", index=False)
    print(f"Saved: {out_dir}/Perovskite_5800data_addln.csv")
    return df_5
...
def analyze_distribution(df_5):
    ...
    for col, fname in [("lnTS80m", f"{fig_dir}/lnTS80m_hist_new.png"),
                        ("TS80m",   f"{fig_dir}/TS80m_hist_new.png")]:
        plt.figure()
    ...
    plt.xlabel("TS80m (h)")
    plt.tight_layout()
    plt.savefig(f"{fig_dir}/TS80m_hist_new_tail_enlarged.png", dpi=300)
    plt.close()

    # Q-Q plots
    for col, fname in [("lnTS80m", f"{fig_dir}/qqplot_lnTS80m.png"),
                        ("TS80m",   f"{fig_dir}/qqplot_TS80m.png")]:
        fig, ax = plt.subplots()
...
def main():
    ...
    csr_path = "outputs/csr/all_sp1_oliynyk_zero_csr.npz"
